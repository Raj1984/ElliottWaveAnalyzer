"""
Elliott Wave impulse scan on Silver (COMEX front-month future, SI=F).

Downloads daily OHLC data from Yahoo Finance via yfinance, converts it to the
column layout this project expects, and uses the WaveAnalyzer engine to brute-
force valid 12345 impulsive moves (and leading diagonals) starting from the
lowest low in the window.

Run:  python example_silver.py
...
TICKER = 'SI=F'        # COMEX silver front-month future
START = '2023-01-01'
...
df.to_csv(r'data\silver_1d.csv', sep=",", index=False)
